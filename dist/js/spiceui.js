...
var DialogCore = function () {
	...
	_createClass(DialogCore, [{
		...
		value: function show() {
			...
			$selector.stop(true, true)[showO.easing](showO.speed, function () {
		...
		value: function hide() {
			...
			$selector.stop(true, true)[hideO.easing](hideO.speed, function () {
		...
		key: '_initActivate',
		value: function _initActivate() {
			var self = this,
			    $selector = self.selector,
			    options = self.options;

			// 关闭按钮
			$('.spice-dialog-close', $selector).on('tap', function () {
				self.hide();
			});
		...
		key: '_initDialogTemplate',
		value: function _initDialogTemplate() {}
	}]);
...
var DropdownCore = function () {
	function DropdownCore(selector, options) {
	...
	_createClass(DropdownCore, [{
		...
		value: function show($self) {
			...
			$showElem.stop(true, true)[showO.easing](showO.speed, function () {
				$selector.trigger('spice' + pluginName + '.show', self);
			});
		}
	}, {
		...
		value: function hide($self) {
			...
			$showElem.stop(true, true)[hideO.easing](hideO.speed, function () {
				$selector.trigger('spice' + pluginName + '.hide', self);
			});
		}
	}, {
		key: '_dropElemTap',
		value: function _dropElemTap() {}
	}, {
		key: '_showElemTap',
		value: function _showElemTap() {}
	}]);
...
var _spiceui = __webpack_require__(11);

var _spiceui2 = _interopRequireDefault(_spiceui);
...
spice.subString = function (str, start, strlen) {
    ...
    for (; i < len; i++) {
        ...
        var _unicode = str.charCodeAt(i);
        num += _unicode > 127 ? 2 : 1;
    }

    // 开始取
    for (; i < len; i++) {
        var _unicode2 = str.charCodeAt(i);
        num += _unicode2 > 127 ? 2 : 1;
...
spice.Dropdown = _dropdown2.default;

// 折叠菜单

spice.Collapse = _collapse2.default;

// 弹出层

spice.Dialog = _dialog2.default;

// 弹出框

spice.Tips = _tips2.default;

// 选项卡

spice.Tab = _tab2.default;

// 轮播

spice.Carousel = _carousel2.default;

// 翻页

spice.Pagination = _pagination2.default;
...
var Carousel = function () {
	...
	_createClass(Carousel, [{
		...
		value: function init() {
			...
			$selector.off('mouseenter.spice.carousel').off('mouseleave.spice.carousel').on('mouseenter.spice.carousel', function () {
				self.removeInterval();
		...
		value: function addDot() {
			...
			$('.spice-carousel-dots a', $selector).off('tap.spice.carousel').on('tap.spice.carousel', function () {
		...
		value: function addArrowBtn() {
			...
			$('.spice-carousel-btn-prev', $selector).off('tap.spice.carousel').on('tap.spice.carousel', function () {
				self.prev();
			});

			$('.spice-carousel-btn-next', $selector).off('tap.spice.carousel').on('tap.spice.carousel', function () {
				self.next();
			});
		}
	}, {
		...
		value: function scrollTo(index) {
			...
			$('.spice-carousel-dots a:eq(' + index + ')', $selector).addClass('spice-active').siblings('a').removeClass('spice-active');
		}
	}, {
		key: 'addInterval',
		...
		value: function addTouch() {
			...
			$selector.on('panstart.spice.carousel', function (e) {
				if (e.originalEvent.pointerType == 'mouse') return false;
				self.removeInterval();
				START_X = +$ul.css('transform').substring().replace(/\s/g, '').split(',')[4];
				DIRECTION = '';
			}).on('panleft.spice.carousel panright.spice.carousel', function (e) {
				if (e.originalEvent.pointerType == 'mouse') return false;
				DIRECTION = e.type;
			...
			}).on('panend.spice.carousel', function (e) {
				if (e.originalEvent.pointerType == 'mouse') return false;
				var curPage = Math.ceil(Math.abs(MOVE_CURRENT_X / $selector.width())) + (loop ? 0 : 1);
				if (DIRECTION == 'panleft') {
					var $curA = $('a:eq(' + (curPage - 1) + ')', $dot);
					if ($curA.length > 0) {
...
var _dropdownCore = __webpack_require__(2);

var _dropdownCore2 = _interopRequireDefault(_dropdownCore);
...
var Collapse = function (_DropdownCore) {
	_inherits(Collapse, _DropdownCore);
	...
	_createClass(Collapse, [{
		...
		value: function _dropElemTap() {
			var self = this,
			    options = self.options;
			self.$dropElem.off('tap.spice.' + self.pluginName).on('tap.spice.' + self.pluginName, function (e) {
		...
		key: '_hideOtherMenu',
		value: function _hideOtherMenu($self) {
			var self = this,
			    options = self.options,
			    selector = self.selector;
			$.each($(options.showElem, selector), function (i, elem) {
				var $otherElem = $(elem).not($self);
...
var _dialogCore = __webpack_require__(1);

var _dialogCore2 = _interopRequireDefault(_dialogCore);
...
var Dialog = function (_DialogCore) {
	_inherits(Dialog, _DialogCore);
	...
	_createClass(Dialog, [{
		key: '_initDialogTemplate',
		value: function _initDialogTemplate(opts) {
			var self = this,
				...
				var buttons = options.buttons || [],
				    buttonsHtml = '';
				if (buttons.length > 0) {
					var buttonsStr = buttons.map(function (item, i) {
...
var Dropdown = function (_DropdownCore) {
	_inherits(Dropdown, _DropdownCore);
	...
	_createClass(Dropdown, [{
		...
		value: function _showElemTap() {
			var self = this;
			self.$showElemTap.off('tap.spice.' + self.pluginName).on('tap.spice.' + self.pluginName, function (e) {
...
module.exports = function (selector, options) {
	...
	$('body').off('tap.spice.dropdown').on('tap.spice.dropdown', function (e) {
		$.each(toggle, function (i, elem) {
			var $elem = $(elem),
...
var eventList = ['tap', 'doubletap', 'pan', 'panstart', 'panmove', 'panend', 'pancancel', 'panleft', 'panright', 'panup', 'pandown', 'swipe', 'swipeleft', 'swiperight', 'swipeup', 'swipedown', 'press', 'pressup', 'pinch', 'pinchstart', 'pinchmove', 'pinchend', 'pinchcancel', 'pinchin', 'pinchout', 'rotate', 'rotatestart', 'rotatemove', 'rotateend', 'rotatecancel'];
	...
	var _loop = function _loop() {
		...
		$.event.special[value] = {
			setup: function setup(data, namespaces, eventHandle) {
				var $self = $(this),
				    hammertime = new _hammerjs2.default($self[0], {
					// cssProps: {
					// 	userSelect: 'inherit'
					// }
				});
				$self.data('hammerjs', hammertime);
				hammertime.on(value, function (e) {
					eventHandle(e);
				});
			},
...
var Pagination = function () {
	...
	_createClass(Pagination, [{
		...
		value: function addEvents() {
			...
			$selector.on('tap.spice.pagination', 'a.spice-page-numbers', function () {
				var $self = $(this),
			...
			// 
			$selector.on('change.spice.pagination', 'select.spice-form-control', function () {
				var $self = $(this),
				    value = +$self.val();
			...
			// 
			$selector.on('keyup.spice.pagination', '.spice-page-skip input', function () {
				var $self = $(this),
				    value = $self.val();
				$self.val(value.replace(/[^0-9]/g, '') || '');
			});

			$selector.on('tap.spice.pagination', '.spice-btn', function () {
				var $self = $(this),
		...
		key: 'setPageSize',
		value: function setPageSize(val) {
			var self = this,
			    $selector = self.selector,
			    options = self.options,
			    total = options.total,
		...
		key: 'refreshButton',
		value: function refreshButton(page) {
			var self = this,
			    $selector = self.selector,
			    options = self.options,
			    pageNumber = self.pageNumber,
		...
		key: 'getPageLimitsTemp',
		value: function getPageLimitsTemp() {
			var self = this,
			    $selector = self.selector,
			...
			var optionTemp = [];
			for (var i = 0; i < pageLimitsList.length; i++) {
				var curLimit = pageLimitsList[i];
...
var Tab = function () {
	...
	_createClass(Tab, [{
		...
		value: function init(opts) {
			...
			self.$titleItem.off('tap.spice.' + self.pluginName).on('tap.spice.' + self.pluginName, function (e) {
				var $self = $(e.currentTarget),
				    index = $self.index();
		...
		value: function show() {
			...
			self.$titleItem.eq(index).addClass(activeClass).siblings(options.titleItem).removeClass(activeClass);

			self.$contentItem.eq(index).show().addClass(activeClass).siblings(options.contentItem).hide().removeClass(activeClass);
		}
	}]);
...
var Tips = function (_DialogCore) {
	_inherits(Tips, _DialogCore);

	function Tips(options) {
		_classCallCheck(this, Tips);

		var _this = _possibleConstructorReturn(this, (Tips.__proto__ || Object.getPrototypeOf(Tips)).call(this));

		_this.pluginName = 'tips';
		_this.options = $.extend(true, {}, _this.options, {
			tipsClass: ''
